"""A2-E2 独立 public coarse MLP 的确定性无门控 baseline。"""
...
from can.model.a2_public_mlp import (
    ...
    A2_PUBLIC_CLASS_NAMES,
    A2_PUBLIC_EXPERIMENT_ID,
    A2_PUBLIC_PARAMETER_COUNT,
    A2_PUBLIC_SOURCE_CLASS_COUNT,
    A2_PUBLIC_SOURCE_TO_COARSE,
    A2_PUBLIC_TEST_CLASS_SUPPORT,
...
PUBLIC_GLOBAL_SEED: Final = 20_260_730
SPLIT_SEED: Final = 20_260_724
PUBLIC_TRAIN_LOADER_SEED: Final = 20_260_731
...
SMOKE_ACCURACY_PERCENT: Final = 90.0
...
class A2PublicDataResource:
    """固定一个 public baseline 复用的 Fashion-MNIST 压缩资源。"""
...
A2_PUBLIC_DATA_RESOURCES: Final = (
    A2PublicDataResource(
...
class A2PublicBaselineError(RuntimeError):
    """表示 A2-E2 public baseline 不满足固定实验协议。"""


@dataclass(frozen=True, slots=True)
class PublicEpochMetrics:
    """记录一个固定 public training epoch 的聚合指标。"""
...
@dataclass(frozen=True, slots=True)
class PublicEvaluationMetrics:
    """记录不暴露单样本 logits 的 public 二分类指标。"""
...
@dataclass(frozen=True, slots=True)
class A2PublicDataBundle:
    """保存 public baseline 的固定 split loaders 与索引摘要。"""
...
def _hash_int64_values(values: torch.Tensor | Sequence[int]) -> str:
    ...
    for value in iterable:
        if type(value) is not int:
            raise A2PublicBaselineError("int64 hash input contains a non-exact integer")
...
def _validate_environment() -> None:
    if platform.system() != "Linux" or platform.machine() != "x86_64":
        raise A2PublicBaselineError("A2-E2 public baseline requires Linux x86_64")
    if platform.python_version_tuple()[:2] != ("3", "11"):
        raise A2PublicBaselineError("A2-E2 public baseline requires CPython 3.11")
    expected_versions = {
        "torch": TORCH_VERSION,
    ...
    for distribution, expected in expected_versions.items():
        if importlib.metadata.version(distribution) != expected:
            raise A2PublicBaselineError(f"unsupported {distribution} version")
    if (
        str(torch.__version__) != TORCH_VERSION
        or str(torchvision.__version__) != TORCHVISION_VERSION
    ):
        raise A2PublicBaselineError("runtime and distribution versions diverged")
    if torch.version.cuda is not None or torch.version.hip is not None or torch.cuda.is_available():
        raise A2PublicBaselineError("A2-E2 public baseline does not support accelerator builds")
    if not torchvision.extension._has_ops():
        raise A2PublicBaselineError("torchvision CPU operators are unavailable")


def _configure_determinism() -> None:
    if os.environ.get("PYTHONHASHSEED") != str(PUBLIC_GLOBAL_SEED):
        raise A2PublicBaselineError(f"PYTHONHASHSEED must equal {PUBLIC_GLOBAL_SEED}")
    random.seed(PUBLIC_GLOBAL_SEED)
    np.random.seed(PUBLIC_GLOBAL_SEED)
    torch.manual_seed(PUBLIC_GLOBAL_SEED)
...
def _validate_data_resources(data_root: Path) -> dict[str, dict[str, str]]:
    ...
    expected_names = {resource_item.filename for resource_item in A2_PUBLIC_DATA_RESOURCES}
    observed_names = {path.name for path in raw_root.glob("*.gz") if path.is_file()}
    if observed_names != expected_names:
        raise A2PublicBaselineError("Fashion-MNIST compressed resource set changed")

    result: dict[str, dict[str, str]] = {}
    for resource_item in A2_PUBLIC_DATA_RESOURCES:
...
def _validate_dataset(dataset: FashionMNIST, *, train: bool) -> None:
    ...
    expected_source_support = (6_000,) * 10 if train else (1_000,) * 10
    if len(dataset) != expected_size:
        raise A2PublicBaselineError("Fashion-MNIST split size changed")
    if (
        ...
        or not dataset.targets.is_contiguous()
        or tuple(dataset.targets.shape) != (expected_size,)
        or int(dataset.targets.min().item()) != 0
        or int(dataset.targets.max().item()) != A2_PUBLIC_SOURCE_CLASS_COUNT - 1
    ):
        raise A2PublicBaselineError("Fashion-MNIST decoded tensor contract changed")
    source_support = tuple(
        int(value)
        for value in torch.bincount(
            dataset.targets, minlength=A2_PUBLIC_SOURCE_CLASS_COUNT
        ).tolist()
    )
    if source_support != expected_source_support:
        raise A2PublicBaselineError("Fashion-MNIST source-label support changed")
    image, source_label = dataset[0]
    validate_a2_public_images(cast(torch.Tensor, image).unsqueeze(0).contiguous())
    if type(source_label) is not int or not 0 <= source_label < A2_PUBLIC_SOURCE_CLASS_COUNT:
        raise A2PublicBaselineError("Fashion-MNIST sample label is non-canonical")
    map_a2_public_labels(torch.tensor([source_label], dtype=torch.int64))


def _load_data(data_root: Path) -> A2PublicDataBundle:
    transform = ToTensor()
    ...
    loader_generator = torch.Generator(device="cpu").manual_seed(PUBLIC_TRAIN_LOADER_SEED)
...
def _train_epoch(
    ...
    loss_sum = 0.0
    sample_count = 0
    for images, source_labels in loader:
        validate_a2_public_images(images)
        labels = map_a2_public_labels(source_labels)
        validate_a2_public_labels(labels, images.shape[0])
    ...
    if sample_count < 1:
        raise A2PublicBaselineError("training loader produced no samples")
    return loss_sum / sample_count


def _train_model(
    data: A2PublicDataBundle, *, emit_progress: bool
) -> tuple[A2FashionMNISTPublicMLP, tuple[PublicEpochMetrics, ...]]:
    model = A2FashionMNISTPublicMLP().to(device="cpu", dtype=torch.float32)
    criterion = nn.CrossEntropyLoss()
    optimizer = Adam(
        model.parameters(),
        lr=0.001,
    ...
    epoch_metrics: list[PublicEpochMetrics] = []
...
def _evaluate(
    model: A2FashionMNISTPublicMLP,
    loader: DataLoader[tuple[torch.Tensor, torch.Tensor]],
) -> PublicEvaluationMetrics:
    model.eval()
    loss_sum = 0.0
    total = 0
    correct = 0
    confusion = torch.zeros((A2_PUBLIC_CLASS_COUNT, A2_PUBLIC_CLASS_COUNT), dtype=torch.int64)
    ...
    with torch.inference_mode():
        for images, source_labels in loader:
            ...
            bins = torch.bincount(
                labels * A2_PUBLIC_CLASS_COUNT + predictions,
                minlength=A2_PUBLIC_CLASS_COUNT * A2_PUBLIC_CLASS_COUNT,
            ).reshape(A2_PUBLIC_CLASS_COUNT, A2_PUBLIC_CLASS_COUNT)
            confusion += bins
    if total < 1:
        raise A2PublicBaselineError("evaluation loader produced no samples")
    ...
    per_class_accuracy = tuple(
        ...
        for index in range(A2_PUBLIC_CLASS_COUNT)
    )
    return PublicEvaluationMetrics(
        loss=loss_sum / total,
...
def _hash_model_state(model: A2FashionMNISTPublicMLP) -> str:
    digest = hashlib.sha256()
...
def _measure_serialized_state(model: A2FashionMNISTPublicMLP) -> tuple[int, str]:
    with tempfile.TemporaryDirectory(prefix="can-a2-public-state-") as directory:
...
def _time_model(
    model: A2FashionMNISTPublicMLP, batches: Sequence[torch.Tensor]
) -> dict[str, float]:
    if not batches:
        raise A2PublicBaselineError("latency batches must not be empty")
    model.eval()
    with torch.inference_mode():
...
def _measure_latency(
    ...
    for index in range(1_000):
        ...
        if type(image) is not torch.Tensor:
            raise A2PublicBaselineError("test transform returned a non-tensor image")
        first_images.append(image)
    stacked = torch.stack(first_images).contiguous()
    validate_a2_public_images(stacked)
...
def _cpu_model_name() -> str:
    ...
    if type(payload) is not dict or type(payload.get("lscpu")) is not list:
        raise A2PublicBaselineError("lscpu JSON has the wrong schema")
    for entry in payload["lscpu"]:
        if type(entry) is dict and entry.get("field") == "Model name:":
            value = entry.get("data")
            if type(value) is str and value:
                return value
    raise A2PublicBaselineError("lscpu JSON does not contain a CPU model")
...
def _write_report(report: dict[str, object], repeat: int) -> Path:
    ...
    output_path = A2_REPORT_ROOT / f"public-baseline-repeat-{repeat}.json"
...
def run_a2_public_baseline(repeat: int) -> Path:
    """执行一次固定 A2-E2 public baseline 并写入 ignored JSON 报告。"""
    if type(repeat) is not int or repeat not in (1, 2):
        raise A2PublicBaselineError("repeat must be exactly 1 or 2")
    ...
    if test_metrics.total != TEST_SIZE:
        raise A2PublicBaselineError("official test sample count changed")
    if test_metrics.per_class_count != A2_PUBLIC_TEST_CLASS_SUPPORT:
        raise A2PublicBaselineError("public test class support changed")
    if test_metrics.accuracy_percent < SMOKE_ACCURACY_PERCENT:
        raise A2PublicBaselineError("A2-E2 public test accuracy is below the smoke floor")
    ...
    if parameter_count != A2_PUBLIC_PARAMETER_COUNT:
        raise A2PublicBaselineError("A2-E2 public parameter count changed")
    ...
    deterministic_payload: dict[str, object] = {
        ...
        "source_to_coarse": A2_PUBLIC_SOURCE_TO_COARSE,
        "epochs": [
            {
                "epoch": item.epoch,
    ...
    report: dict[str, object] = {
        ...
        "data": {
            ...
            "validation_indices_sha256": data.validation_indices_sha256,
            "source_to_coarse": A2_PUBLIC_SOURCE_TO_COARSE,
            "public_class_names": A2_PUBLIC_CLASS_NAMES,
            "test_class_support": A2_PUBLIC_TEST_CLASS_SUPPORT,
        },
        "training": {
            "global_seed": PUBLIC_GLOBAL_SEED,
            "split_seed": SPLIT_SEED,
            "loader_seed": PUBLIC_TRAIN_LOADER_SEED,
            "optimizer": "Adam",
            "learning_rate": 0.001,
            "betas": (0.9, 0.999),
            "epsilon": 1e-8,
...
def compare_a2_public_repeats() -> str:
    """比较两份 public baseline 报告并返回一致的确定性指纹。"""
    fingerprints: list[str] = []
    for repeat in (1, 2):
        path = A2_REPORT_ROOT / f"public-baseline-repeat-{repeat}.json"
        try:
            payload = json.loads(path.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError) as error:
            raise A2PublicBaselineError("A2-E2 public report is missing or malformed") from error
        if type(payload) is not dict or set(payload) != {
            ...
            raise A2PublicBaselineError("A2-E2 public report schema changed")
        fingerprint = payload.get("determinism_fingerprint")
        if (
            payload.get("schema_version") != 1
            or payload.get("experiment_id") != A2_PUBLIC_EXPERIMENT_ID
            or payload.get("repeat") != repeat
            or type(fingerprint) is not str
            or len(fingerprint) != 64
        ):
            raise A2PublicBaselineError("A2-E2 public report identity changed")
        fingerprints.append(fingerprint)
    if fingerprints[0] != fingerprints[1]:
        raise A2PublicBaselineError("A2-E2 public repeated runs are not deterministic")
    return fingerprints[0]


def main(argv: Sequence[str] | None = None) -> int:
    """解析固定 repeat/compare 命令并运行 A2-E2 public baseline。"""
    parser = argparse.ArgumentParser(description="Run the fixed CAN A2-E2 public CPU baseline")
    mode = parser.add_mutually_exclusive_group(required=True)
    mode.add_argument("--repeat", type=int, choices=(1, 2))
    mode.add_argument("--compare", action="store_true")
    arguments = parser.parse_args(argv)
    if arguments.compare:
        fingerprint = compare_a2_public_repeats()
        print(json.dumps({"determinism_fingerprint": fingerprint}, sort_keys=True))
        return 0
    output_path = run_a2_public_baseline(cast(int, arguments.repeat))
